...
public class SqliteDeviceTableTest {
	...
	public void testGetAdded() {
		...
		assertTrue(d != null);
		assertTrue(Calendar.getInstance().get(Calendar.YEAR) >= 2011);
	}
	...
	public void testGetSetActive() {
		...
		assertTrue(deviceTable.isActive(1));
		assertTrue(deviceTable.isActive(2));
	...
	public void testRowExists() {
		...
		assertTrue(deviceTable.rowExists(INTERNAL_ID_1));
		assertFalse(deviceTable.rowExists(INTERNAL_ID_3));
	}

	@Test
	public void testAdded() {
		// This test contains a bunch of timing tests
		// i.e. this test mig fail once in a while.
		addSomeRows();

		assertTrue(deviceTable.rowExists(INTERNAL_ID_1));

		Date d = deviceTable.getAdded(ID_1);
		long diff = d.getTime()-new Date().getTime();
		assertTrue(diff  < 40 && diff > -40);
		try {
			Thread.sleep(200);
		...
		assertTrue(diff  > 180 && diff < 220);
		
		deviceTable.setAdded(ID_1,new Date());
		d = deviceTable.getAdded(ID_1);
		diff = d.getTime()-new Date().getTime();
		assertTrue(diff  < 40 && diff > -40);
	}

	private void addSomeRows() {
		ID_1 = deviceTable.createRow(INTERNAL_ID_1, TYPE_1);
		ID_2 = deviceTable.createRow(INTERNAL_ID_2, TYPE_2);
	}
}
