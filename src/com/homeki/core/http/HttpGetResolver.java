...
public class HttpGetResolver {
	...
	public static void resolve(StringTokenizer st, HttpApi api,
		...
		if (!st.hasMoreTokens()) {
			...
			if (token.equals("get") || token.equals("set")) {
				...
				HttpMacro.send404(
						"Missing tokens. Expected get or set action.", out);
			}
		}
	}
	...
	private static void resolveGetStatus(StringTokenizer st, HttpApi api,
			DataOutputStream out) throws IOException {
	...
	private static void resolveGetDim(StringTokenizer st, HttpApi api,
			DataOutputStream out) throws IOException {
		HashMap<String, String> args = getArguments(st);
		Integer id = HttpArguments.demandInteger("id", args, out);
		...
		if (id != null && level != null) {
			try {
				...
				HttpMacro.send404("The device with id " + id
						+ " is not dimmable.", out);
				return;
			}
	...
	private static void resolveGetOn(StringTokenizer st, HttpApi api,
			DataOutputStream out, boolean on) throws IOException {
		HashMap<String, String> args = getArguments(st);
		Integer id = HttpArguments.demandInteger("id", args, out);
		if (id == null) {
			...
			try {
				...
				HttpMacro.send404("The device with id " + id
						+ " is not switchable.", out);
				return;
			}
			HttpMacro.sendResponse(200, "Device " + id + " is now "
					+ (on ? "on" : "off") + ".", out);
		}
	}

	private static void resolveGetDevices(StringTokenizer st, HttpApi api,
			DataOutputStream out) throws IOException {
		HttpMacro.sendResponse(200, api.getDevices(), out);
	}

	private static void resolveGetTime(StringTokenizer st, HttpApi api,
			DataOutputStream out) throws IOException {
		HttpMacro.sendResponse(200, "Time is: " + (new Date()).toString(), out);
	}

	private static void resolveGetHistory(StringTokenizer st, HttpApi api,
			DataOutputStream out) throws IOException {
		HashMap<String, String> args = getArguments(st);
	...
	private static void resolveGetEcho(StringTokenizer st, HttpApi api,
			DataOutputStream out) throws IOException {
		if (!st.hasMoreTokens())
